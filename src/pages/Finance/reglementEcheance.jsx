...
import AjoutReglementEcheance from "../../components/AddReglementEcheanceForm";
import EditReglementEcheance from "../../components/AddReglementEcheanceForm/edit";
...
function ReglementEcheance() {
    ...
    return (
        <Container>
                ...
                <Table sx={{ minWidth: 400, margin: '20px' }} size={'small'}>
                    <TableHead>
                        <TableRow>
                            ...
                            <StyledTableCell><span>rc_montant</span></StyledTableCell>
                    ...
                    <TableBody>
                        {data.map((row) =>(
                            <TableRow hover={true}>
                                ...
                                <RowTableCell>{row.rc_montant}</RowTableCell>
                                <RowTableCell>{row.rg_type_reg}</RowTableCell>
                                

                                <RowTableCell>
                                    <Link to={`${url}/${row.rg_no}`}><FontAwesomeIcon icon={ faArrowUpRightFromSquare } className='details-icon'/></Link>
                                </RowTableCell>
                                <RowTableCell>
                                    <ActionButtonEdit>
                                        <FontAwesomeIcon onClick={() => {setId(row.rg_no);setOpenedit(true)}} icon={ faPenToSquare } className='btn btn-edit' />
                                    </ActionButtonEdit>
                                    <ActionButtonDelete>
                                        <FontAwesomeIcon onClick={() => deleteData(row.rg_no)} icon={ faTrashCan } className='btn btn-delete' />
                                    </ActionButtonDelete>
                                </RowTableCell>
                            </TableRow>
                        ))}
            ...
            <AjoutReglementEcheance open={open} setOpen={setOpen} />
            <EditReglementEcheance openedit={openedit} setOpenedit={setOpenedit} id={id} />
        </Container>
    )
}

export default ReglementEcheance